...
if strcmp(test,'Harvey') || strcmp(test,'all')
    model.rxns = regexprep(model.rxns,'\[bc\]','\(e\)');
    model.rxns = regexprep(model.rxns,'\(bc\)','\(e\)');
    model.rxns = regexprep(model.rxns,'\[a\]','\(e\)');
    model.rxns = regexprep(model.rxns,'\(a\)','\(e\)');
    model.rxns = regexprep(model.rxns,'\[csf\]','\(e\)');
    model.rxns = regexprep(model.rxns,'\(csf\)','\(e\)');
    model.rxns(strmatch('biomass_reactionIEC01b_trtr',model.rxns)) = {'biomass_maintenance'};
    model.rxns(strmatch('biomass_reactionIEC01b',model.rxns)) ={ 'biomass_reaction'};
end

if saveDiary
    %save each diary to PSCM_toolbox/Files/OrganChecks/
    aPath=which('MethodSection3.mlx');
    aPath=strrep(aPath,'MethodSection3.mlx',['Files' filesep 'OrganChecks' filesep]);
    diary([aPath filesep 'Test4Functions_diary.txt']);
end
TestedRxns =[];
tol = 1e-6;
...
if strcmp(test,'Recon1') || strcmp(test,'all') || strcmp(test,'Harvey')
    ...
    % %  if ~isnan(TestSolution(k,1)); TestedRxns = [TestedRxns; model.rxns(find(abs(FBA.v)>tol))]; end ;k = k +1;clear FBA
    ...
    if find(model.c)>0
        try
            FBA=entropicFluxBalanceAnalysis(model,param);
    ...
    TestSolutionName{k,1} = 'Test human biomass reaction';
    if ~isnan(TestSolution(k,1)); TestedRxns = [TestedRxns; model.rxns(find(abs(FBA.v)>tol))]; end ;k = k +1;clear FBA
    %% "Human Recon 1 human  biomass"
    model = modelOri;
    model.c(find(model.c)) = 0;
    ...
    TestSolutionName{k,1} = 'Test human biomass reaction (noTrTr)';
    if ~isnan(TestSolution(k,1)); TestedRxns = [TestedRxns; model.rxns(find(abs(FBA.v)>tol))]; end ;k = k +1;clear FBA
    %% "Human Recon 1 human  biomass"
    model = modelOri;
    model.c(find(model.c)) = 0;
    ...
    TestSolutionName{k,1} = 'Test human biomass reaction (maintenance)';
    if ~isnan(TestSolution(k,1)); TestedRxns = [TestedRxns; model.rxns(find(abs(FBA.v)>tol))]; end ;k = k +1;clear FBA
...
if strcmp(test,'IECOri')
    ...
    try
        FBA=entropicFluxBalanceAnalysis(model,'min',param);
        FBA.f=sum(FBA.v(find(model.c>0)));
    catch
        FBA.f=NaN;
    end
...
if strcmp(test,'IEC') || strcmp(test,'all')|| strcmp(test,'Harvey')
    ...
    TestSolutionName{k,1} = 'putriscine to methionine (depends on oxygen uptake) - UNK2';
    if ~isnan(TestSolution(k,1)); if ~isnan(TestSolution(k,1)); TestedRxns = [TestedRxns; model.rxns(find(abs(FBA.v)>tol))]; end ; end ;k = k +1;clear FBA
